...
def merge_mcp_config(settings_file: Path, new_config: dict) -> None:
    ...
    if settings_file.exists():
        try:
            ...
            print(f"Warning: Existing {settings_file} is not valid JSON, overwriting", file=sys.stderr)
            existing_config = {}
        ...
        for hook_type, hook_configs in new_config['hooks'].items():
            ...
            for new_hook_config in hook_configs:
                ...
                for existing_hook in existing_config['hooks'][hook_type]:
                    if (existing_hook.get('matcher') == new_hook_config.get('matcher') and
                        existing_hook.get('hooks', [{}])[0].get('command') ==
                        new_hook_config.get('hooks', [{}])[0].get('command')):
                        exists = True
                        break
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: merge_settings.py <settings_file> <new_config_json>", file=sys.stderr)
        sys.exit(1)
